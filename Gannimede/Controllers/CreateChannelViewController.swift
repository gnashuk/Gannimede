        ...
        if let title = titleTextField.text, !title.isEmpty {
            if isChannelTitleUnique(title: title) {
                let alert = UIAlertController(
                    title: LocalizedStrings.AlertTitles.ConfirmCreation,
                    message: String.localizedStringWithFormat(LocalizedStrings.AlertMessages.ConfirmCreation, title),
                    preferredStyle: .alert
                )
                alert.addAction(UIAlertAction(title: LocalizedStrings.AlertActions.Confirm, style: .default) { [weak self] action in
                    if let newChannelReference = self?.channelsReference.childByAutoId(), let user = self?.user {
                        let channelValue: [String: Any] = [
                            "title" : title,
                            "ownerId": user.uid,
                            "description": self?.channelDescription ?? "",
                            "isPrivate": self?.privateSwitch.isOn ?? false
                        ]
                        newChannelReference.setValue(channelValue)
                        
                        self?.userChannelIds.append(newChannelReference.key!)
                        self?.usersReference.child(user.uid).child("channelIds").setValue(self?.userChannelIds)
                        self?.sendUserInvitation(channelId: newChannelReference.key!, channelTitle: title)
                        self?.performSegue(withIdentifier: "Creation Done", sender: sender)
                    }
                })
